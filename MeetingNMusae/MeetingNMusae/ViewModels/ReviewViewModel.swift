...
//    }
    ...
    func fetchData(roomCode: String) {
        db.collection("reivews").whereField("room_code", isEqualTo: "\(roomCode)").addSnapshotListener { (querySnapshot, error) in
...
//            _ = try db.collection("reivews").document("\(review.roomCode)").setData(from: review)
            _ = try db.collection("reivews").addDocument(from: review)
        } catch {
            print(error)
            return
        }
